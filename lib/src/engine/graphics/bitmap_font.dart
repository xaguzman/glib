...
class BitmapFont extends Disposable {
...
//   * [region] The texture region containing the glyphs. The glyphs must be relative to the lower left corner (ie, the region
//   *           should not be flipped). If the region is null the glyph images are loaded from the image path in the font file.
//   * [flip] If true, the glyphs will be flipped for use with a perspective where 0,0 is the upper left corner. */
//  BitmapFont (FileHandle fontFile, TextureRegion region, [bool flip = false]) {
...
//  BitmapFont (FileHandle fontFile, bool flip) {
...
//   * ignored.
//   * @param flip If true, the glyphs will be flipped for use with a perspective where 0,0 is the upper left corner. */
//  BitmapFont (FileHandle fontFile, FileHandle imageFile, bool flip) {
...
//  BitmapFont (FileHandle fontFile, FileHandle imageFile, bool flip, bool integer) {
...
//  }
  ...
  /**
   * Constructs a new BitmapFont from the given [BitmapFontData] and array of [TextureRegion]. If the TextureRegion
   ...
   */
  BitmapFont(BitmapFontData data, List<TextureRegion> pageRegions,
      bool integer) {
   ...
   * [BitmapFontCache#addText(String, float, float)]
   */

  GlyphLayout draw(Batch batch, String str, float x, float y) {
    cache.clear();
  ...
  /** Draws text at the specified position.
   * @see BitmapFontCache#addText(String, float, float, int, int, float, int, bool, String) */
  GlyphLayout draw(Batch batch, String str, float x, float y,
      float targetWidth, int halign, bool wrap) {
    cache.clear();
  ...
  /** Draws text at the specified position.
   * @see BitmapFontCache#addText(String, float, float, int, int, float, int, bool, String) */
  GlyphLayout draw(Batch batch, String str, float x, float y, int start,
      int end, float targetWidth, int halign,
      bool wrap) {
    cache.clear();
    GlyphLayout layout = cache.addText(
        str,
        x,
        y,
        start,
  ...
  /** Draws text at the specified position.
   * @see BitmapFontCache#addText(String, float, float, int, int, float, int, bool, String) */
  void draw(Batch batch, GlyphLayout layout, float x, float y) {
    cache.clear();
  ...
  void setFixedWidthGlyphs(String glyphs) {
  ...
  void setUseIntegerPositions(bool integer) {
    this.integer = integer;
...
class BitmapFontData
{
  ...
  List<List<Glyph>> glyphs = new List<List<Glyph>>(PAGES);
  ...
  BitmapFontData (FileHandle fontFile, bool flip) {
    this.fontFile = fontFile;
    this.flipped = flip;
    load(fontFile, flip);
  }

  void load (FileHandle fontFile, bool flip) {
    ...
    try {
      ...
      List<String> padding = line.substring(0, line.indexOf(' ')).split(",", 4);
      ...
      List<String> common = line.split(" ", 7); // At most we want the 6th element; i.e. "page=N"
      ...
      imagePaths = new List<String>(pageCount);
      ...
      for (int p = 0; p < pageCount; p++) {
        ...
        List<String> pageLine = line.split(" ", 4);
      ...
      Glyph capGlyph = null;
      for (String capChar in capChars) {
   ...
   * {@link #getGlyphs(GlyphRun, String, int, int, bool)} should be be used to shape a string of characters into a
   * list of glyphs. */
   ...
   *           width. */
  void getGlyphs (GlyphRun run, String str, int start, int end, bool tightBounds) {
  ...
  int getWrapIndex (List<Glyph> glyphs, int start) {
  int i = start - 1;
  for (; i >= 1; i--)
...
class Glyph {
  ...
  int getKerning (char ch) {
    if (kerning != null) {
      List<byte> page = kerning[ch >> LOG2_PAGE_SIZE];
      if (page != null) return page[ch & PAGE_SIZE - 1];
    }
    return 0;
  }
  ...
  void setKerning (int ch, int value) {
    ...
    List<byte> page = kerning[ch >> LOG2_PAGE_SIZE];
    if (page == null)
      kerning[ch >> LOG2_PAGE_SIZE] = page = new List<byte>(PAGE_SIZE);
